...
public abstract class LazyFragment_5 extends Fragment {
    ...
    protected void dispatchChildVisibleState(boolean state) {
        ...
        if (fragments != null) {
            for (Fragment fragment : fragments) { // 循环遍历 嵌套里面的 子 Fragment 来分发事件操作
                if (fragment instanceof LazyFragment_5 &&
                        !fragment.isHidden() &&
                        fragment.getUserVisibleHint()) {
                    ((LazyFragment_5) fragment).dispatchUserVisibleHint(state);
                }
            }
        }
    }
    ...
    private boolean isParentInvisible() {
        ...
        if (parentFragment instanceof LazyFragment_5) {
            LazyFragment_5 fragment = (LazyFragment_5) parentFragment;
    ...
    // 工具相关而已
    public void setFragmentDelegater(FragmentDelegate fragmentDelegate) {
